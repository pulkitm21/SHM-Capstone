...
def handle_status(topic: str, payload_raw: str):
    try:
        ...
        cmd_ack = payload.get("cmd_ack")  # NEW
        ...
        # This ensures UI reflects recording/configured/etc
        # ----------------------------
        update_accelerometer_runtime_state(
            node_id=node_id,
            current_state=state,
        ...
        if seq_ack > 0:
            ...
            if pending_seq == seq_ack and odr_index is not None and range_code is not None:
                apply_accelerometer_config_ack(
                    ...
                    hpf_corner=desired_hpf,  # firmware doesn't send yet
                    seq_ack=seq_ack,
                    acked_at=acked_at,
                    current_state=state,
                )
